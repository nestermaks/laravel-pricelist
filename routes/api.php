...
Route::prefix(config('pricelist.api'))->group(function () {
    ...
    Route::post(config('pricelist.pricelists') . '/relation', [BaseController::class, 'relation']);
    Route::get(config('pricelist.pricelists') . '/get/{model_name}/{model_id}', [BaseController::class, 'getPricelistsOfModel']);
    Route::post(config('pricelist.pricelists') . '/model', [BaseController::class, 'relationWithModel']);
});